...
class DataFormatter:
    ...
    def _format_bar_data(self, results, question):
        ...
        if len(results[0]) == 2:
            ...
            # Use LLM to get a relevant label
            prompt = ChatPromptTemplate.from_messages([
                ("system", r'''You are a data labeling expert. Given a question and some data, provide a concise and relevant label for the data series. 
                 Only give answers in Brazilian Portuguese.
                 '''),
                ("human", "Question: {question}\nData (first few rows): {data}\n\nProvide a concise label for this y axis. For example, if the data is the sales figures for products, the label could be 'Sales'. If the data is the population of cities, the label could be 'Population'. If the data is the revenue by region, the label could be 'Revenue'. Only provide labels in Brazilian Portuguese."),
            ])
            label = self.llm_manager.invoke(prompt, question=question, data=str(results[:2]))
        ...
        print(f"Raw results: {formatted_data}")
        return {"formatted_data_for_visualization": formatted_data}
    ...
    def _format_other_visualizations(self, visualization, question, sql_query, results):
        ...
        instructions = graph_instructions.get(clean_visualization, "Unknown visualization type")

        # instructions = graph_instructions[visualization.strip()]
        print(f"Visualization key3: '{visualization}'")
        print(f"Available keys: {list(graph_instructions.keys())}")
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("human", 'For the given question: {question}\n\nSQL query: {sql_query}\n\nResult: {results}\n\nFormat the response as a valid JSON object matching this example: {instructions}. ONLY return JSON, without Markdown, extra characters, or explanations.'),
        ...
        try:
            ...
            # Extract 'data' key if it exists
            if isinstance(formatted_data_for_visualization, dict) and "data" in formatted_data_for_visualization:
                formatted_data_for_visualization = formatted_data_for_visualization["data"]
            
            return {"formatted_data_for_visualization": formatted_data_for_visualization}
        except json.JSONDecodeError as e:
            return {"error": "Failed to format data for visualization", "raw_response": response, "exception": str(e)}